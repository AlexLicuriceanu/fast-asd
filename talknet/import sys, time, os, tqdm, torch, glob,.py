...
def initialize_detector(device='cuda'):
    model = YOLO("/home/rhc/licenta4/models/yolov8n-face.pt")
    model.to(device)
    return model
...
def extract_frames_sequential(video_path, frame_numbers):
    ...
    total = max(frame_numbers)

    while cap.isOpened() and current_frame_number <= total:
        ret, frame = cap.read()

        if not ret:
            break
        
        if current_frame_number == frame_numbers[current_idx]:
            frames.append(frame)
            current_idx += 1
            if current_idx == len(frame_numbers):
                break
...
def main(
    ...
    frame_numbers = [int(start_frame + i * (fps / 25)) for i in range(int(duration * 25))]
    frames = extract_frames_sequential(video_path, frame_numbers)
