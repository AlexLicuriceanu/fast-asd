...
def predict_faces(DET, pyframesPath, pyworkPath, start_frame=0, batch_size=16):
...
def predict_faces_from_frames(DET, frames, pyworkPath, start_frame=0, batch_size=16):
    dets = []
...
def crop_video(track, cropFile, pyframesPath, audioFilePath, start_frame = 0):
    ...
    avg_s = numpy.mean(dets['s']) * (1 + 0.40)
...
def main(
    ...
    for ii, track in enumerate(allTracks):
        if (end_seconds - start_seconds) * fps < STORE_FRAMES_IN_MEMORY_THRESHOLD:
            vidTracks.append(crop_video_from_frames(track, os.path.join(pycropPath, '%05d'%ii), frames, audioFilePath))
        else:
            vidTracks.append(crop_video(track, os.path.join(pycropPath, '%05d'%ii), pyframesPath, audioFilePath))
